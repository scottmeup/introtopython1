#http://www.codeskulptor.org/#user40_pM2byAIPwV_68.py
...
def draw(canvas):
    global score1, score2, paddle1_pos, paddle2_pos, ball_pos, ball_vel, left_pad_pos, right_pad_pos, left_pad_vel, right_pad_vel, pad_deflection, pad_max_deflection
    ...
    if ball_pos[0] <= 0 + pad_width + ball_radius:
        ...
        if (ball_pos[1] > left_pad_pos) and (ball_pos[1] < left_pad_pos + pad_height):
            ball_vel[0] *= -1.1
            if left_pad_pos + pad_height == 0:
                pass
            else:
                ball_vel[1] += spin(ball_pos[1], left_pad_pos, pad_height, pad_deflection)
                print "Vertical speed", ball_vel[1]
        else:
            #miss left paddle
            score2 += 1
            spawn_ball(right)
    ...
    if ball_pos[0] >= width - pad_width - ball_radius:
        ...
        if (ball_pos[1] > right_pad_pos) and (ball_pos[1] < right_pad_pos + pad_height):
            ball_vel[0] *= -1.1
            if right_pad_pos + pad_height == 0:
                pass
            else:
                ball_vel[1] += spin(ball_pos[1], right_pad_pos, pad_height, pad_deflection)
                print "Vertical speed", ball_vel[1]
        else:
            #miss right paddle
            score1 += 1
    ...
    #set a limit on the "spin" a player can put on the ball vertically
    if ball_vel[1] > pad_max_deflection:
        ball_vel[1] = pad_max_deflection
    if ball_vel[1] < (pad_max_deflection * -1):
        ball_vel[1] = (pad_max_deflection * -1)
    
    # draw scores
    score_height = 48
    canvas.draw_text(str(score1) + " : "  + str(score2), ((width/2 - score_height), score_height + 10), score_height, "White")
    
def spin(ball_pos, pad_top, pad_height, deflection):
    #Hopefully this will be a value between -0.5 and 0.5
    direction = ((float(ball_pos) / float(pad_top + pad_height)) - 0.5)
    print "Direction", direction
    spin = direction * deflection
    print "Spin", spin
    return spin
